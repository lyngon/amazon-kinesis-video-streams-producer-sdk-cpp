...
#define APP_NAME "kvs_gstreamer_file_uploader_sample" SI
#define LOG_INFO(fmt, ...)                                                \
    do                                                                    \
    {                                                                     \
        fprintf(stdout, "[INFO] " APP_NAME ": " fmt "\n", ##__VA_ARGS__); \
    } while (0)
#define LOG_ERROR(fmt, ...)                                                \
    do                                                                     \
    {                                                                      \
        fprintf(stderr, "[ERROR] " APP_NAME ": " fmt "\n", ##__VA_ARGS__); \
    } while (0)
...
{
    _CustomData() : stream_status(STREAM_STATUS_OK) {}
    int stream_status;
    int content_type;
    string file_path;
...
int gstreamer_init(int argc, char *argv[], CustomData *data)
{
    ...
    if (data->content_type == CONTENT_TYPE_VIDEO_ONLY)
    { // video only
